function idx_force_disp = sync_data(vic_pip,inst_data,ext_data)
    ...
    %
    % Inputs
    %     vic_pip       VIC-3D data structure with PIPCount and Time fields
    %     inst_data     Instron data structure with PIPCount and Time fields
    %     ext_data      Extensometer data structure with Index field
    % Outputs
    %     idx_force_disp  Index vector for synchronizing force-displacement data
    % Methodology
    %     1. Matches data lengths between VIC and extensometer data
    %     2. Finds matching points using PIPCount
    %     3. Adjusts VIC time to match Instron time
    %     4. Trims negative time and extra data
    %     5. Creates index vector for force-displacement synchronization
    % Dependencies
    %     None
