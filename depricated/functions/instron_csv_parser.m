function data = instron_csv_parser(filename,NumHeaderLines,VariableNamesLine,VariableUnitsLine)
%{
    ...
    Inputs:
        ...
        VariableUnitsLine   (OPTIONAL) integer, 3 by default
    
    Outputs:
        data                table data type containing the data from the
                            csv, variable names and units
...
%}
    ...
    if ~exist('VariableUnitsLine','var')
        VariableUnitsLine = 3;
    end
    ...
    data = readtable(filename,"NumHeaderLines",NumHeaderLines,"VariableNamesLine",VariableNamesLine,"VariableUnitsLine",VariableUnitsLine);
    data = addprop(data,{'FileName','TestDate'},{'table','table'});
